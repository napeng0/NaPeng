...
HumanView::HumanView(shared_ptr<IRenderer> renderer)
{
	...
	if (renderer)
	{
		...
		m_pCamera.reset(New CameraNode(&Mat4x4::s_Identity, frustum));
...
void HumanView::RegisterAllDelegates(void)
{
	...
	pGlobalEventManager->VAddListener(MakeDelegate(this, &HumanView::PlaySoundDelegate), EvtData_PlaySound::s_EventType);
}

void HumanView::RemoveAllDelegates(void)
{
	IEventManager* pGlobalEventManager = IEventManager::Get();
	pGlobalEventManager->VRemoveListener(MakeDelegate(this, &HumanView::PlaySoundDelegate), EvtData_PlaySound::s_EventType);
}
...
void HumanView::Console::Render()
{
	...
	// Display the output text just below the input text.
...
void HumanView::Console::HandleKeyboardInput(const unsigned int keyVal, const unsigned int oemKeyVal, const bool bKeyDown)
{
	...
	//See if it's a key press that we care about.
	switch (oemKeyVal)
	{
	case VK_BACK:
	{
