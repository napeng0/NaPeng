...
{
	return shared_ptr<IResourceLoader>(New SdkMeshResourceLoader());
}
...
{
	// The raw data of the SDK Mesh file is needed by the CDXUTMesh class
	return rawSize;
}
...
bool SdkMeshResourceLoader::VLoadResource(char *rawBuffer, unsigned int rawSize, shared_ptr<ResHandle> handle)
{
	...
	{
		ASSERT(0 && "This is not supported in D3D9");
	}
	else if (renderer == GameCodeApp::RENDERER_D3D11)
	{
		shared_ptr<D3DSdkMeshResourceExtraData11> extra = shared_ptr<D3DSdkMeshResourceExtraData11>(New D3DSdkMeshResourceExtraData11());
	...
	ASSERT(0 && "Unsupported Renderer in SdkMeshResourceLoader::VLoadResource");
	return false;
}
...
HRESULT D3DMeshNode9::VOnRestore(Scene *pScene)
{
	...
	// exactly the model we're loading. 
	...
	// on those vertices so it will improve performance.     
...
HRESULT D3DTeapotMeshNode9::VOnRestore(Scene *pScene)
{
	...
	Mat4x4 rotateY90;
	rotateY90.BuildRotationY(-PI / 2.0f);
